...
const Coverage = () => {
  ...
  //console.log(serviceCeners);
  const handleSearch= e=>{
    e.preventDefault();
    const location=e.target.location.value;
    const district=serviceCeners.find(c => c.district.toLowerCase().includes(location.toLowerCase()));
    if(district){
      ...
      <div className="border w-full h-[800px]">
        ...
        center={position}
         zoom={8}
          ...
          />
         {
            serviceCeners.map((center,index) =>  <Marker
            key={index}
             position={[center.latitude,center.longitude]}>
            <Popup>
              
              <strong>{center.district}</strong> <br /> Service Area: {center.
covered_area.join(', ')}.
            </Popup>
          </Marker>)
         }
